...
import { addWorkItemWatcherSchema } from "@/lib/validation/api";
...
export async function POST(
  ...
  const parseResult = addWorkItemWatcherSchema.safeParse(await request.json());
    ...
    return apiError(
      {
        ...
        message: error?.message ?? "Unable to add watcher.",
      },
      requestId,
      500
    );
...
export async function DELETE(
  ...
  const { error } = await context.supabase
    ...
    .eq("org_id", context.orgId)
    .eq("work_item_id", workItemId)
    .eq("user_id", parseResult.data.userId);
